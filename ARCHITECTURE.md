...
- **Integration runs in-process.** The merge-train executor spawns a dedicated integration-worker subprocess per cycle to run rebase, post-rebase `ci_check`, and `git merge --force-with-lease` asynchronously. A marker row plus a startup reconciler handle crash windows between `git merge` and the DB transition. Verify-shaped failures (verify, `ci_check` pre-verify or post-rebase, rebase) all route to `replanning` with a typed `VerifyIssue[]`.
