...
| --- | --- | --- | --- |
| Borrow "conceptual minimalism" framing — write a one-pager explaining gvc0's merge train as "queue + workers + tiers" | open | — | No dedicated explainer doc yet. Useful for onboarding and positioning copy. |
| Conflict-tier disclosure — consolidate same-feature lock + cross-feature primary/secondary into a single numbered policy page | open | — | `docs/operations/conflict-coordination.md` covers the mechanics but is not a numbered single-page policy analogous to Overstory's tier list. |
| Read Overstory's conflict-tier source; cite as prior art in `docs/operations/conflict-coordination.md` if tier 3/4 matches primary/secondary | open | — | Highest-payoff research follow-up; blocked only on reading time. |
| Read Overstory's queue → CI → merge sequence; confirm whether they validate against `main` SHA as gvc0 does | open | — | gvc0's rebase → post-rebase CI → SHA validation → `merge --no-ff` chain is implemented (4168a87, b8f37d7); comparison with Overstory's sequence not yet done. |
| Read Overstory's SQLite ↔ git reconciler; compare with gvc0's startup reconciler | open | — | gvc0 has its own startup reconciler (4168a87); cross-comparison with Overstory's approach not yet performed. |
| Look for dependency-shape tracking in Overstory source | open | — | Would be relevant to compare against gvc0's feature-only-on-feature / task-only-within-feature DAG constraints. |
