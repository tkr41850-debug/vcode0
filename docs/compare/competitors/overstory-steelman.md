...
Tracks which follow-up recommendations from this doc have been acted on. One row per recommendation from the "Recommended follow-up reading" section. Update when status changes; reference the commit so reviewers can audit scope.

| Rec | Status | Commit | Notes |
| --- | --- | --- | --- |
| R1 — Close Worker Runaway concern (wall-clock cap + progress-idle timeout) | open | — | Directly addresses STEELMAN #2 and #12; [Worker Runaway](../../concerns/worker-runaway.md) documents the gap. Prerequisite for turning theater into partial on #2. |
| R2 — Close Summarize Retry Loop concern (retry cap + backoff fix) | open | — | [Summarize Retry Loop](../../concerns/summarize-retry-loop.md) admits flat 1s backoff with no cap. Addresses STEELMAN #12. |
| R3 — Add output-throughput metric (merged-to-main lines-per-token-spent) | open | — | [Warnings](../../operations/warnings.md) tracks coordination signals only; no implementation throughput signal. Addresses STEELMAN #8. No feature-candidate yet. |
| R4 — Document cross-task semantic conflict gap (#6b) | open | — | Either accept as design boundary in [conflict-coordination.md](../../operations/conflict-coordination.md) or describe planned mitigation (cross-task type-shape contracts). No feature-candidate yet. |
| R5 — Treat discuss/research budget-mode skip as known regression | open | — | Strongest architectural defense (#5) is conditional on non-budget mode. Add explicit trade-off note in [budget-and-model-routing.md](../../architecture/budget-and-model-routing.md) or create minimum-viable-discuss path. No feature-candidate yet. |
| R6 — Review overstory hooks and watchdog source | deferred | — | Research task to sharpen the [overstory.md](./overstory.md) comparison. No code change implied; defer until overstory comparison is next revisited. |
...
- Companion: [Landscape Overview]](../landscape/OVERVIEW.md)
