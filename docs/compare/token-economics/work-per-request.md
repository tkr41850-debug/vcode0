...
| --- | --- | --- | --- |
| R1 — Whole-codebase / whole-doc analysis replacing RAG pipeline | open | — | gvc0 uses tool-mediated JIT retrieval today; skeleton index prerequisite (tokenmaxxing R7) not yet built. |
| R2 — Reasoning mode + thinking budget at `high`/`xhigh` | open | — | Extended thinking not yet wired per-phase; see tokenmaxxing R6. |
| R3 — AgentWrite-style structured long output (plan-first anti-truncation) | open | — | No length-contract, section-word-count, or end-sentinel patterns in planner/worker prompts. |
| R4 — In-call parallel sampling K=5 with diversity enforcement | open | — | Not applicable to gvc0's current single-path planner architecture; relevant if multi-candidate planning explored. |
| R5 — Multi-task batching K=5–10 with NDJSON-per-slot resilience and order-effect mitigations | partial | — | Planner already produces multi-task plan output in one call; NDJSON-per-slot resilience, BPE voting, and order-effect restatement at end not applied. |
| R6 — Many-shot ICL (200–1000 shots) | open | — | No ICL shot injection in any phase prompt; applicable to classify/extract subtasks if added. |
| R7 — Mega-prompt with constraint stack | open | — | Worker and planner prompts exist but do not apply the restate-top-rules-at-end degeneration mitigation. |
| R8 — In-call agentic loop (deterministic tools, ≤5 steps) | rejected | — | gvc0 explicitly chose multi-call process-per-task isolation; §9 of the doc itself documents this inverse relationship. The failure-mode list (non-deterministic tools, side effects, >5 steps) restates the design rationale. |
| R9 — BSM for multi-criterion evaluation | open | — | No parallel-branches evaluation pattern in verify phase. |
| R10 — External-rubric CoVe (not intrinsic self-refine) | open | — | `outcomeVerification` field provides an external rubric anchor but CoVe independence mechanic (answer verification questions without seeing draft) not enforced in verify prompt. |
