...
- **Cross-family review is empirically supported, not just stylistic.** arXiv 2604.19049 makes the case for pinning the replanner to a different model family than the planner. See [cross-family-replanner.md](../../feature-candidates/coordination/cross-family-replanner.md).
...
| --- | --- | --- | --- |
| Resist multiplying task workers beyond what the dependency graph unblocks | done | — | DAG-only scheduler enforces this structurally; no unconstrained worker fan-out path exists. |
| Replanner parallelism stays at one | done | — | Single replanner-agent class by design; no concurrent replanner path in the codebase. |
| Cross-feature coordination stays asymmetric (primary/secondary, not peer voting) | done | e9e54d4 | `e9e54d4` serializes secondary-feature suspension on cross-feature overlap; symmetric peer pattern is never exercised. |
| Pin replanner to a different model family than the planner (cross-family review) | deferred | — | See [cross-family-replanner.md](../../feature-candidates/coordination/cross-family-replanner.md). |
| Deprecate "multi-agent" framing in README/marketing copy | open | — | README still uses multi-agent language; small copy change. |
