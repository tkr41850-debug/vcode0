...
The full evaluation record is in [absurd-evaluation.md](../../feature-candidates/interop/absurd-evaluation.md).
...
This is one of several reasons LangGraph remains a "migration substrate candidate" rather than a "migration substrate recommendation" in [OVERVIEW.md]](../landscape/OVERVIEW.md).
...
| --- | --- | --- | --- |
| Reject direct adoption of `absurd` (four structural blockers) | done | c641478 | Decision documented; `c641478` lands the full evaluation. |
| Borrow `absurd.sql` schema patterns into SQLite journal (per-attempt runs rows, checkpoint table, first-write-wins events, lease expiry) | deferred | — | See [absurd-pattern-borrow.md](../../feature-candidates/interop/absurd-pattern-borrow.md). Revisit when gvc0 next reshapes its persistence schema. |
| Keep git refs as authoritative state; do not split authority with a durable engine | done | — | Structural property of the current implementation; no action needed beyond the documented decision. |
| Track Temporal LLM-agent workflow patterns when published (Q3 2026) | open | — | Monitor item; no implementation work until patterns are public. |
