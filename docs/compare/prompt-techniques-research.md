...
| R9 — Tighten non-standard tool descriptions | partial | d38afaf | d38afaf: raiseIssue, addFeature, addTask, addDependency, submit (planner), dependency parameters, plus minimalist half — compressed worker-tool descriptions on read_file / write_file / edit_file / list_files / search_files / git_status / git_diff. Follow-up: submitDiscuss / submitResearch / submitSummarize / submitVerify / confirm — flipped from terse one-liner to cardinality + downstream-consumer + post-condition; confirm switched from orchestrator-mechanics framing to worker decision criteria; submitVerify description now matches the schema enum (`pass` / `replan_needed`, no stale "repair-needed"). Phase-host inspection tools (getFeatureState / listFeatureTasks / listFeatureEvents / etc.) still on the original terse line. |
