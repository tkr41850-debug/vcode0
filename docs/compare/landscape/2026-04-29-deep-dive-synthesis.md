...
Synthesis of five parallel deep-dive subagent investigations completed 2026-04-29, building on the [2026-04-28 landscape overview](./OVERVIEW.md) and the four [adjacent-scan subagents]](../lineage/gsd-2.md) on Overstory, Factory.ai, GSD-2, and LangGraph.
...
- [Serial vs. Parallel Evidence]](../research/serial-vs-parallel-evidence.md) — empirical case against broad fine-grained parallelism (arXiv 2511.00872, 2512.08296, 2604.19049).
- [Durable Execution]](../research/durable-execution.md) — Temporal / Restate / Inngest / DBOS / Earendil's `absurd`; where gvc0 sits relative to dedicated durable workflow engines.
- [Verification Architectures]](../research/verification-architectures.md) — cross-family review evidence, SWE-Bench Verified flaws, SARIF as interop surface, how other coding agents verify.
- [AGENTS.md & AAIF]](../research/agents-md-and-aaif.md) — Linux Foundation Agentic AI Foundation (Dec 2025) + AGENTS.md as the cross-vendor convention; implications for gvc0's CLAUDE.md-only stance.
- [Pi Ecosystem Post-Earendil]](../research/pi-ecosystem-post-earendil.md) — Earendil as pi-sdk steward (April 2026); `gondolin` microVM sandbox; `absurd` Postgres-native workflow engine; RFC 0015 licensing.
...
gvc0 currently respects only `CLAUDE.md`. Adding AGENTS.md interop is small and avoids a slow drift toward looking parochial relative to the rest of the ecosystem. See [agents-md-interop.md](../../feature-candidates/interop/agents-md-interop.md).
...
**Update (2026-04-29 follow-up):** the `absurd` evaluation is closed. Direct adoption rejected (TS SDK incompatible with process-per-task; FIFO scheduler with no DAG; pi-agent checkpointing shallower than gvc0's IPC journal; Postgres-only). What survives is pattern-borrowing — `absurd.sql` as a reference spec when gvc0 next reshapes its SQLite journal. See [absurd-evaluation.md](../../feature-candidates/interop/absurd-evaluation.md), [absurd-pattern-borrow.md](../../feature-candidates/interop/absurd-pattern-borrow.md), and [durable-execution.md]](../research/durable-execution.md).

### 6. gvc0's moat survives the survey

Three properties remain genuinely distinctive after the deep dive:

- **Typed `VerifyIssue` + dedicated replanner agent.** No public competitor has both. Adding optional SARIF output ([sarif-verifyissue-output.md](../../feature-candidates/interop/sarif-verifyissue-output.md)) makes this interoperable without diluting the typed model.
...
- **Programmatic merge train.** Only Overstory shares the shape, and Overstory's queue lacks the rebase → post-rebase-CI → main-SHA validation → `merge --no-ff` invariant chain. Worth auditing Overstory's issue #103 for the analogous bug class — see [merge-step-file-filtering-audit.md](../../feature-candidates/coordination/merge-step-file-filtering-audit.md).

## Ten prioritized recommendations

### Now (1–2 sprints, high ROI)

1. **Pin replanner to a different model family than the planner.** Smallest change in this list with the largest quality lever. Evidence: arXiv 2604.19049. Spec: [cross-family-replanner.md](../../feature-candidates/coordination/cross-family-replanner.md).
2. **Add AGENTS.md interop alongside CLAUDE.md.** Cheap; closes a perception gap relative to the AAIF-backed convention. Spec: [agents-md-interop.md](../../feature-candidates/interop/agents-md-interop.md).
3. **Audit Overstory issue #103 (merge step file filtering) against gvc0's merge train.** Concrete bug class to verify we don't share. Spec: [merge-step-file-filtering-audit.md](../../feature-candidates/coordination/merge-step-file-filtering-audit.md).

### Soon (2–4 sprints, real value)

4. **Add optional SARIF output mode for `VerifyIssue` payloads.** Interop with GitHub Code Scanning + IDE extensions. Spec: [sarif-verifyissue-output.md](../../feature-candidates/interop/sarif-verifyissue-output.md).
5. **~~Evaluate Earendil's `absurd` for orchestrator scheduling state.~~** *Done 2026-04-29.* Direct adoption rejected; pattern-borrow path tracked at [absurd-pattern-borrow.md](../../feature-candidates/interop/absurd-pattern-borrow.md). Outcome recorded in [absurd-evaluation.md](../../feature-candidates/interop/absurd-evaluation.md).
6. **Plan the harness-boundary abstraction even though `ClaudeCodeHarness` stays deferred.** Substrate hedge per [claude-code-harness.md](../../feature-candidates/runtime/claude-code-harness.md). The Earendil finding lowers urgency but does not invalidate the abstraction.
...
9. **Watch Factory.ai for any coordinator state-machine specification.** Currently opaque; a public spec would change the differentiation pitch in [factory-ai.md]](../competitors/factory-ai.md).
...
| --- | --- | --- | --- |
| R1 — Pin replanner to a different model family than the planner | open | — | Spec: [cross-family-replanner.md](../../feature-candidates/coordination/cross-family-replanner.md). Highest-leverage finding per the synthesis. |
| R2 — Add AGENTS.md interop alongside CLAUDE.md | open | — | Spec: [agents-md-interop.md](../../feature-candidates/interop/agents-md-interop.md). |
| R3 — Audit Overstory issue #103 (merge step file filtering) against gvc0's merge train | open | — | Spec: [merge-step-file-filtering-audit.md](../../feature-candidates/coordination/merge-step-file-filtering-audit.md). One-time investigation; output is a documented finding. |
| R4 — Add optional SARIF output mode for `VerifyIssue` payloads | open | — | Spec: [sarif-verifyissue-output.md](../../feature-candidates/interop/sarif-verifyissue-output.md). |
| R5 — ~~Evaluate Earendil's `absurd` for orchestrator scheduling state~~ | done | c641478 | Direct adoption rejected (TS SDK incompatible, FIFO-only scheduler, shallower checkpointing, Postgres-only). Pattern-borrow path tracked at [absurd-pattern-borrow.md](../../feature-candidates/interop/absurd-pattern-borrow.md); evaluation record at [absurd-evaluation.md](../../feature-candidates/interop/absurd-evaluation.md). |
| R6 — Plan the harness-boundary abstraction (ClaudeCodeHarness stays deferred; abstraction hedge is the goal) | open | — | Spec: [claude-code-harness.md](../../feature-candidates/runtime/claude-code-harness.md). Earendil finding lowered urgency but did not invalidate the abstraction. |
| R7 — Watch AAIF MCP / AGENTS.md / goose stewardship cadence | open | — | Watching brief; trigger: AAIF publishes a coordination spec overlapping gvc0's split state model. |
| R8 — Watch `absurd` public release notes for SQLite mode or Pi-sdk DAG case study | open | — | Watching brief; direct adoption remains closed until structural blockers resolve. |
| R9 — Watch Factory.ai for any coordinator state-machine specification | open | — | Watching brief; currently opaque. |
| R10 — Watch SWE-Bench Pro adoption as de facto benchmark | open | — | Watching brief; OpenAI Feb 2026 audit found 59.4% of SWE-Bench Verified problems had test-design issues. |
