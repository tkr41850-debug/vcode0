...
**In:** `squashMergeTaskIntoFeature` operation; `events.ts` task-success restructure (squash before `transitionTask('merged')`); capped retry loop with `maxSquashRetries` config; `inbox_items` table + Store methods (full CHECK union shipped up-front); add `'awaiting_merge'` to `POST_EXECUTION_PHASES` so `featurePhaseCategory` returns `'post'` instead of `'done'`; `VerifyIssueSource` extended with `'squash'`.

**Out:** task-worktree disposal (Phase 4); separate `SquashVerifyIssue` type (reuse `RebaseVerifyIssue` with `source: 'squash'`); cross-feature dependency rebase; reconciler-driven merge replay; CHECK widening migrations (table ships full union from the start).
...
- `src/orchestrator/conflicts/git.ts` — add a **new** sibling to the existing `rebaseTaskWorktree` (`:14-23`, which is rebase-only and does not squash): `squashMergeTaskIntoFeature(taskBranch: string, featureBranch: string, featureWorktreePath: string, commitMessage: string): Promise<{ ok: true; sha: string } | { ok: false; conflict: true; conflictedFiles: string[] }>`. Implementation: `simpleGit(featureWorktreePath)` → `checkout featureBranch` (idempotent) → `merge --squash <taskBranch>` → on conflict, capture conflicted files via `status()`, `merge --abort`, and return conflict; on success, `commit -m <message>` and return the new HEAD sha. The `--squash` form leaves a single staged change; the explicit `commit` produces the feature-branch commit. `rebaseTaskWorktree` remains untouched — it is the conflict-resolution primitive Step 5.2 calls between squash retries. **Resolving `featureWorktreePath` at the events.ts call site**: the handler does not receive a path directly. Compute it via `path.join(projectRoot, worktreePath(feature.featureBranch))` (the same idiom used for task worktrees at `harness/index.ts:212`). `projectRoot` is threaded into the scheduler ctor via `OrchestratorPorts.config.projectRoot` (already populated in `compose.ts:42-49`); store it on the scheduler instance so `events.ts` handlers can read it without re-injecting through every call. The feature worktree is already ensured by `dispatch.ts:459-460` before any task in the feature dispatches, so no fresh `ensureFeatureWorktree` call is needed at submit time — the directory exists.
- `src/orchestrator/scheduler/events.ts:84-105` — today the `taskLanded` handler calls `transitionTask(..., status='done', collabControl='merged')` at `:87`, then `features.onTaskLanded`, then `conflicts.reconcileSameFeatureTasks`. **Restructure**: first call `squashMergeTaskIntoFeature` (task still in pre-merge state); only call `transitionTask(..., status: 'done', collabControl: 'merged')` on `{ ok: true }`; then `features.onTaskLanded`. **On conflict**: leave both axes unchanged (keep `status: 'running'`); Step 5.2's loop mutates neither until it resolves. FSM (`src/core/fsm/index.ts:349`) allows `running → done` on retry success but has no `done → running` arc — do not flip `done` early. On exhaustion, Step 5.2 calls `transitionTask({status: 'failed'})` alongside inbox+reroute. Do **not** route through `reconcileSameFeatureTasks` — its filter (`same-feature.ts:65-75`) only matches `suspended/same_feature_overlap` tasks; a just-landed conflict matches none. Call `squashMergeTaskIntoFeature` and `rebaseTaskWorktree` directly (Step 5.2).
...
- Update `test/integration/feature-phase-agent-flow.test.ts` if it asserts feature-branch tip equals main (it shouldn't after this change). Note that the `WorktreeProvisioner` stub at `:65-66` already carries `ensureFeatureBranch` from Phase 8 step 8.2 by the time Phase 5 ships — leave that stub method untouched.
...
> **Schema-up-front callout (read first):** the `inbox_items` migration shipped here defines the full CHECK union from the start: `CHECK (kind IN ('squash_retry_exhausted', 'semantic_failure', 'retry_exhausted', 'destructive_action'))`. Per the baseline's pre-0.0.0 stance, downstream Phase 1 steps (1.6, 1.7) widen only the TS literal union — they do **not** ship widening migrations.
...
- `test/integration/task-squash-retry-cap.test.ts` — script the rebase to keep producing conflicts (e.g. by holding a phantom conflicting commit on the feature branch that the rebase cannot resolve); assert retries cap at `maxSquashRetries`, no `transitionTask(..., 'merged')` fires, an `inbox_items` row appears with `kind: 'squash_retry_exhausted'` (the canonical Phase 5 value — do NOT substitute `'retry_exhausted'`, which is owned by Phase 1.6 for worker-error retry exhaustion), and the feature ends in `replan_needed` with `source: 'squash'`.
...
> Verify squash retry: (1) retry runs `rebaseTaskWorktree` then `squashMergeTaskIntoFeature` directly — never `reconcileSameFeatureTasks` (filter mismatch silently no-ops); (2) cap honored at `maxSquashRetries` (default 3, threaded through `OrchestratorPorts.config` into the scheduler); exhaustion calls (a) `transitionTask({status: 'failed'})`, (b) `appendInboxItem` (kind `'squash_retry_exhausted'`), (c) `rerouteToReplan` with `source: 'squash'` (NOT `'rebase'`) + `conflictedFiles` from last attempt, and (d) `completeTaskRun` to close the agent_run row; (3) `transitionTask(..., 'merged')` only on success; `features.onTaskLanded` gated on success; (4) rebase target is current feature tip, re-resolved each iteration; (5) `VerifyIssueSource` union extended with `'squash'` (Phase 1 must continue to read this union — cross-phase compatibility); (6) per-attempt log includes attempt N/M. Under 400 words.
...
- **Verify all three callsites of `featurePhaseCategory` in `src/core/scheduling/index.ts`** — `:167`, `:294`, `:600` (the function is module-private, declared at `:119` without `export`; `grep -rn featurePhaseCategory src/` returns only the declaration + 3 callers, so this is the complete consumer set). The expected baseline is that all three are safe to flip without per-site guards — the merge-train view already treats `awaiting_merge` as in-flight, and the scheduling view should match. For each callsite, trace what changes when `awaiting_merge` returns `'post'` instead of `'done'`: (a) does it still terminate scheduling decisions correctly? (b) does it correctly include the feature in the in-flight frontier? (c) does milestone-completion still fire only when every feature reaches a true terminal state (`merged`, not `awaiting_merge`)? If any callsite genuinely needs to treat `awaiting_merge` as terminal (e.g. a UI summary that hides queued features), add an explicit per-site check (`status === 'merged'`) rather than reverting the category. Document any such per-site exception in a one-line comment at the callsite.
