...
- Depends on: phase-0-migration-consolidation (pins migration id 002 for this track and the consolidated `001_init.ts` baseline)
...
- Scope:
  - In:
    ...
    - `workers` table (migration `002`) plus `WorkerRegistryPort` and a SQLite-backed implementation.
