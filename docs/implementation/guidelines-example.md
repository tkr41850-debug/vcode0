...
Files:
  - `src/runtime/ipc/frames.ts` (add variant + zod schema)
  - `src/persistence/sql/012_help_request_kind.sql` (new migration)
  - `src/orchestrator/host/help-router.ts` (new file, ~80 LOC)
Tests:
...
Rollback: revert; migration is additive so down-migration unnecessary, but `012_help_request_kind` will need manual drop on databases that already migrated.
...
Files:
  - `src/agents/planner/prompt-scaffold.ts:204` (remove stdout site)
  - `src/agents/planner/tools/request-help.ts` (rewrite to emit frame)
  - `src/tui/event-bus.ts:312` (delete prefix grep + dead path)
  - `test/integration/planner-help.spec.ts` (rewire assertions)
Tests:
...
Rollback: revert this commit; 9.1 stays. Planner reverts to stdout path and TUI prefix-grep returns. Verify by re-running `examples/scripted-help-loop.ts` and seeing the legacy path engage.
Behavior diff:
  - before: planner blocks on `console.error("HELP: <q>")`; TUI greps prefix from event log; non-TUI runs lose the signal.
  - after: planner emits `help_request` frame; host persists + fans out; non-TUI runs surface via event bus subscribers.
