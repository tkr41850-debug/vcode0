...
> Verify escalation, split into the wiring slice and the prompt slice. **Wiring (TDD slice):** (1) `buildFeaturePhaseAgentToolset` extension wires `request_help` into the discuss toolset when a help-response callback is supplied; (2) `runtime.ts:359-371` constructs the discuss agent with the matching help-callback (either reusing `LiveProposalPhaseSession` or via a sibling `LiveDiscussPhaseSession` — note which); (3) the discuss faux-model test reaches `await_response` and resumes via `respondToHelp`, then `submitDiscuss` completes. **Prompt (prose slice):** (4) discuss / plan / replan prompts include the escalation paragraph; (5) discuss is identified as the primary capture point; (6) `docs/agent-prompts/{discuss-feature.md,plan-feature.md,README.md}` mirrors are synced. **Boundaries:** (7) no new wait state introduced; (8) no inbox row is written by Step 7.1 (Step 7.2 owns that). Under 400 words.
