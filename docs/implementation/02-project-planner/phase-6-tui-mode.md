...
Depends on: phase-5-bootstrap-rewrite (auto-enter consumes the greenfield bootstrap result and startup wiring), phase-4-project-planner-agent (TUI project sessions, proposal review, and session lifecycle depend on the project-planner run model)
Default verify: npm run check:fix && npm run check
Phase exit: all four commits land in order; `npm run verify` passes; greenfield bootstrap attaches the operator to the auto-spawned project-planner session at startup; `/project` opens the session picker mid-project; the composer focus indicator always shows the destination scope; project-proposal approval surfaces handle topology, running-task impact, and stale-baseline rebase paths; final review confirms the TUI surface is coherent and operator guidance in `docs/reference/tui.md` is up to date.
Doc-sweep deferred: none

## Contract

Goal: Add an explicit TUI mode for project-planner chat with auto-enter on greenfield bootstrap, persistent session list, and a composer focus indicator that always shows which scope keystrokes route to.

The composer chrome change is the smaller half; the chat surface is the larger half but reuses existing proposal-mirror and chat infrastructure.

Scope:
  In: project-planner mode entry/exit; auto-enter on greenfield bootstrap result from `phase-5-bootstrap-rewrite`; composer focus indicator chrome with mode label (extends today's `ComposerStatus`); session list view (resume / start new); approval surface for project proposals (new graph-diff component, since none exists today); cancellation-approval step for topology changes affecting running tasks.
...
- `src/tui/composer-autocomplete.ts` and `src/tui/app-navigation.ts` already implement the focus-indicator chrome groundwork from `01-baseline` phase `phase-7-composer-focus-and-autocomplete`. Step 6.1 extends, not replaces.
...
- Session list rendering will pull from `Store.listProjectSessions` (`phase-2-agent-runs-scope`). No equivalent exists on `main`; this is a `phase-2-agent-runs-scope` dependency.
- `docs/reference/tui.md` documents the current TUI surfaces; needs an additive update.
...
### Step 6.1 — Composer focus indicator [risk: low, size: L]
...
`esc` semantics: Step 6.1 only changes the label; it does not change `esc` behavior. The existing `esc` behavior from `01-baseline` phase `phase-7-composer-focus-and-autocomplete` (defocus composer back to graph focus) is unchanged. Mode exit (leaving project-planner mode) is owned by Step 6.2 and is a separate keystroke / slash command — `esc` defocuses, it does not detach the session.
...
Review goals:
1. Verify the chrome line is always visible.
2. Verify the label text changes per mode.
3. Verify a defocused composer still shows the most recent destination.
4. Verify the view-model derivation is testable in isolation.
...
### Step 6.2 — Project-planner mode entry/exit + slash command [risk: med, size: L]
...
- Pick "start new" creates a session via the coordinator (`phase-4-project-planner-agent`) and attaches.
...
Review goals:
1. Verify the slash command registers.
2. Verify the session list pulls from the `phase-2-agent-runs-scope` `Store` helper.
3. Verify entry attaches the proposal mirror and composer to the right session.
4. Verify `esc` does not cancel the session.
5. Verify cancellation is a separate explicit action.
Word cap: under 350 words.

Commit: `feat(tui/project-planner): mode entry and session picker`

---

### Step 6.3 — Auto-enter on greenfield bootstrap [risk: med, size: L]
...
What: consume the bootstrap result from `phase-5-bootstrap-rewrite`. If the result is `{ kind: 'greenfield-bootstrap', sessionId }`, the TUI initializes directly into project-planner mode attached to that session. The composer chrome shows `composer · project planner: <session-id>`. The user is in chat from the first frame.

Dependency: today `initializeProjectGraph(...)` returns `{ milestoneId, featureId }` and `src/compose.ts` constructs `TuiApp` with no bootstrap result. `phase-5-bootstrap-rewrite` introduces the `greenfield-bootstrap` shape and the auto-spawn; Step 6.3 cannot land before `phase-5-bootstrap-rewrite`.
...
- `src/tui/app-deps.ts` — `phase-5-bootstrap-rewrite` already changes `initializeProject(...)`'s return type to `{ kind: 'greenfield-bootstrap', sessionId } | { kind: 'existing' }`. Step 6.3 routes that signal into the deps surface as a `bootstrapResult` field that the view-model reads at startup. `phase-5-bootstrap-rewrite` owns the type change; Step 6.3 owns the consumption.
...
Review goals:
1. Verify the greenfield bootstrap result is consumed by the TUI.
2. Verify project-planner mode is the initial mode in greenfield.
3. Verify existing-project startup is unchanged.
4. Verify the chrome label correctly identifies the auto-spawned session.
Word cap: under 250 words.
...
### Step 6.4 — Project proposal approval + cancellation-approval surface [risk: high, size: L]
...
- Approval of a `removeFeature(f-3)` proposal where `f-3` has a running task surfaces the cancellation-approval block; approve-without-cancel rejects with the `phase-4-project-planner-agent` `ProposalRebaseReason` (`kind: 'running-tasks-affected'`); approve-with-cancel cancels the task via `cancelFeatureRunWork` and then applies.
...
- Stale-baseline rejection (`phase-4-project-planner-agent` `ProposalRebaseReason` of `kind: 'stale-baseline'`) re-opens the session with a clear system message rendered by `proposal-review.ts`.
...
3. Verify approve-without-cancel cleanly rejects.
4. Verify approve-with-cancel applies both the cancellation and the topology change.
5. Verify the stale-baseline path re-opens the session.
Word cap: under 400 words.
