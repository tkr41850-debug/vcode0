...
> Verify the toolset split: (1) `createFeaturePlanToolset` and `createProjectPlannerToolset` are defined as named exports (replacing today's `createPlannerToolset`); (2) `editFeatureSpec` rejects rename / topology-adjacent fields (rename `name` rejection in this commit; `milestoneId` rejection deferred to Phase 4 when the field lands); (3) scope-aware dependency tools reject out-of-scope edges; (4) `buildProposalAgentToolset`'s scope arg defaults to `'feature'` so plan/replan call sites still compile pre-Step-3.2 (Step 3.2 then makes the scope explicit and adds the integration test); (5) `discuss`/`research`/`verify`/`summarize`/`execute-task` construction is untouched **by Phase 3** — Phase 7 separately extends the discuss toolset with `request_help` for topology escalation; that is not in scope here. Under 350 words.
