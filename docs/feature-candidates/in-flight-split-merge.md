...
The baseline `splitFeature` and `mergeFeatures` operations only work on features in pre-planning phases (`discussing`, `researching`) — before any tasks have been created. At this stage, splitting redistributes description and dependency edges only; tasks are planned fresh on each resulting sub-feature. Merging unions descriptions and dependency edges.
...
A later version could support splitting and merging features that already have tasks — including features in `planning`, `executing`, or `replanning` phases. This would enable mid-flight replanning scenarios where a feature turns out to be larger or smaller than expected.

Key challenges for in-flight split:
- Redistributing existing tasks across sub-features (the current `SplitSpec.taskIds` field anticipates this)
...
The baseline pre-planning guard captures the main replanning need (restructure before committing to task-level work) without taking on the full complexity cost.
