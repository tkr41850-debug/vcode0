...
- feature-level verification runs the full configured feature command list
...
### Reuse verified feature results when merge-train input is unchanged
If a merge-train branch is effectively the same verified feature SHA and no meaningful code changes were introduced by rebase, the system could reuse prior feature-level verification results instead of rerunning the entire suite.
...
If a command's relevant inputs are unchanged between feature verification and merge-train verification, the system could skip or reuse that command's result.
