...
- `feature_ci` runs the full configured heavy feature command list before agent-level spec review
...
If a merge-train branch is effectively the same verified feature SHA and no meaningful code changes were introduced by rebase, the system could reuse prior `feature_ci` results instead of rerunning the entire suite.
...
If a command's relevant inputs are unchanged between `feature_ci` and merge-train verification, the system could skip or reuse that command's result.
