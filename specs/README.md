...
Use this page as the canonical scenario index. For testing strategy and faux-provider guidance, see [docs/operations/testing.md](../docs/operations/testing.md).
...
- [test_feature_verification_replan_loop](./test_feature_verification_replan_loop.md) — failed pre-queue `ci_check` and agent-level `verifying` both route through `replanning` with persisted `VerifyIssue[]` before queue entry.
