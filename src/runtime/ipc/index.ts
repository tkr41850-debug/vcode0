...
/**
 * Worker-side NDJSON transport. Mirror of {@link NdjsonStdioTransport} but
 * with the types flipped: it receives {@link OrchestratorToWorkerMessage}
 * from the parent and sends {@link WorkerToOrchestratorMessage} back.
 */
export class NdjsonWorkerStdioTransport implements WorkerIpcTransport {
  private buffer = '';
  private closed = false;
  private messageHandler:
    | ((message: OrchestratorToWorkerMessage) => void)
    | undefined;
  private readonly onData = (chunk: string | Buffer): void => {
    this.buffer += chunk.toString();
    ...
    while (true) {
      ...
      this.messageHandler?.(JSON.parse(line) as OrchestratorToWorkerMessage);
    }
  };
